...
class CharacteristicsCallbacks : public BLECharacteristicCallbacks
{
    void onWrite(BLECharacteristic *characteristic)
    {
        ...
        if (data.length() <= 23)
        {
            ...
            if (error)
            {
                Serial.print("deserializeJson() failed prima deserializare: ");
            ...
            for (auto item : doc.as<JsonArray>())
            {
                ...
                std::string nume = "{\"id\": \"" + idMeal + "\", \"name\": \"" + strMeal + "\", \"image\": \"" + strMealThumb + "\"}";
                characteristic->setValue(nume);
        ...
        else
        {
            ...
            if (error)
            {
                Serial.print("deserializeJson() failed a doua deserializare: ");
                Serial.println(error.c_str());
            }
            ...
            error = deserializeJson(documentquery, client);
            if (error)
            {
                Serial.print("deserializeJson() failed: a treia deserializare");
                Serial.println(error.c_str());
                return;
            }
